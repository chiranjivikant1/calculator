...
input1.setAttribute("onkeyup","check(event)");


	
function check(event){
  if(event.code=="Digit0" || event.code=="Digit1" ||  event.code=="Digit2" ||  event.code=="Digit3" ||  event.code=="Digit4" ||  event.code=="Digit5" ||  event.code=="Digit6" ||  event.code=="Digit7" ||  event.code=="Digit8" ||  event.code=="Digit9" ||  event.code=="Digit0" ||  event.code=="Backspace" ||  event.code=="Minus"   ){}
  else
    {
...
document.querySelector("#equal").innerText= "=";
      ...
      if(checkzero(val))
		...
		function solve()
		{
			...
			let y = eval(x);
      if(y==undefined){ document.getElementById("input1").value =0}
        else
